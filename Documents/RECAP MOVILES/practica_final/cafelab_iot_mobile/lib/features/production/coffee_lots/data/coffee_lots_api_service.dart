...
class CoffeeLotsApiService {
  ...
  Future<CoffeeLotResponseDto> create(CreateCoffeeLotRequestDto dto) async {
  ...
  Future<List<CoffeeLotResponseDto>> getAll() async {
  ...
  Future<http.Response> _send({
    ...
    debugPrint('[CoffeeLotsApiService] $method $uri');
    debugPrint(
      '[CoffeeLotsApiService] auth present: ${headers.containsKey(HttpHeaders.authorizationHeader)}',
    );
    ...
    debugPrint('[CoffeeLotsApiService] response: ${response.body}');
    return response;
  }

  List<CoffeeLotResponseDto> _parseList(String body) {
    if (body.trim().isEmpty) return [];
    final decoded = jsonDecode(body);
    if (decoded is! List) return [];
    return decoded
        .whereType<Map<String, dynamic>>()
        .map(CoffeeLotResponseDto.fromJson)
        .toList();
  }
